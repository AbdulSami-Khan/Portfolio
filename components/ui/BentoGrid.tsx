...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid md:auto-rows-[14rem] grid-cols-1 md:grid-cols-7 gap-4 max-w-7xl mx-auto ",
        className
      )}
    >
      {children}
...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('abdulsamikhan396@gmail.com');
    setCopied(true)
  }
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 lg:gap-4">
                {["Next.JS", "Typescript", "TailwindCSS"].map((item) => (
                  <span
                    key={item}
                ...
                <span className="py-4 rounded-lg px-3 text-center bg-[#10132E]" />
              </div>

              <div className="flex flex-col gap-3 lg:gap-4">
              <span className="py-4 rounded-lg px-3 text-center bg-[#10132E]" />
                {["PostgreSQL", "Prisma", "React Hooks"].map((item) => (
                  <span
                    key={item}
                    className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]"
                  >
